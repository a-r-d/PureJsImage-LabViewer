...
describe('science domain profile', () => {
  it('describes the current science workbench without geo capabilities', () => {
    ...
    expect(profile.agentPolicy.decisionFor('analysis.execute')).toBe('allow')
    expect(profile.agentPolicy.decisionFor('model.preview')).toBe('allow')
    expect(profile.agentPolicy.decisionFor('network.explicit-hosts')).toBe('deny')
  ...
  it('wires identifiable science action handlers through injected ports', async () => {
    ...
    const handlers = createScienceActionHandlers({
      ...
      omeZarrStoreDescription: () => ({ available: false }),
      omeZarrDatasetList: () => [],
      omeZarrDatasetDescription: () => ({ available: false }),
      omeZarrStorageDescription: () => ({}),
      omeZarrNetworkDescription: () => ({ available: false }),
