...
test('keeps the agent opt-in and its OpenRouter key session-memory-only', async ({ page }) => {
  ...
  await settings.getByLabel('Tool-capable model').selectOption('custom')
  await settings.getByLabel('Custom model ID').fill('vendor/custom-tool-model')
  await expect(panel.getByRole('button', { name: 'Start task' })).toBeDisabled()
...
test('supports keyboard navigation, swipe adjustment, and blink timing', async ({ page }) => {
  ...
  await expect(page.locator('[data-atlas-settled="true"]')).toBeVisible({ timeout: 45_000 })
  await expect(page.getByRole('button', { name: 'Swipe' })).toBeVisible({ timeout: 15_000 })
